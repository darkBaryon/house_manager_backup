...
func (s *Service) Login(ctx context.Context, input LoginInput) (*LoginResult, error) {
	...
	slog.InfoContext(ctx, "admin.auth.login.start", "phone", applog.Phone(phone))

	if phone == "" || password == "" {
		err := errcode.InvalidParam.WithError(fmt.Errorf("请输入手机号和密码"))
		applog.Result(ctx, "admin.auth.login.success", "admin.auth.login.failed", err, "phone", applog.Phone(phone))
		return nil, err
	}
	...
	if err != nil {
		...
		applog.Result(ctx, "admin.auth.login.success", "admin.auth.login.failed", dbErr, "phone", applog.Phone(phone))
		return nil, dbErr
	}
	if staff == nil {
		authErr := errcode.Unauthorized.WithError(fmt.Errorf("手机号或密码错误，请重新输入"))
		applog.Result(ctx, "admin.auth.login.success", "admin.auth.login.failed", authErr, "phone", applog.Phone(phone))
		return nil, authErr
	}
	...
	if err != nil {
		dbErr := publicSystemError(errcode.DatabaseError.Code, "登录失败，请稍后重试", err)
		applog.Result(ctx, "admin.auth.login.success", "admin.auth.login.failed", dbErr, "staff_id", staff.ID.Hex())
		return nil, dbErr
	}
	if authRecord == nil || bcrypt.CompareHashAndPassword([]byte(authRecord.PasswordHash), []byte(password)) != nil {
		authErr := errcode.Unauthorized.WithError(fmt.Errorf("手机号或密码错误，请重新输入"))
		applog.Result(ctx, "admin.auth.login.success", "admin.auth.login.failed", authErr, "staff_id", staff.ID.Hex())
		return nil, authErr
	}

	authSession, err := s.buildSession(ctx, staff, "登录失败，请稍后重试")
	if err != nil {
		applog.Result(ctx, "admin.auth.login.success", "admin.auth.login.failed", err, "staff_id", staff.ID.Hex())
		return nil, err
	}
	...
	if err != nil {
		...
		applog.Result(ctx, "admin.auth.login.success", "admin.auth.login.failed", cacheErr, "staff_id", staff.ID.Hex())
		return nil, cacheErr
	}

	if err := s.staffAuthRepo.TouchLastLogin(ctx, authRecord.ID, input.LoginIP); err != nil {
		applog.Result(ctx, "admin.auth.login.success", "admin.auth.login.failed", errcode.DatabaseError.WithError(err), "staff_id", staff.ID.Hex(), "step", "touch_last_login")
	}
	if err := s.writeLoginLog(ctx, staff.ID, input.LoginIP, input.UserAgent); err != nil {
		applog.Result(ctx, "admin.auth.login.success", "admin.auth.login.failed", errcode.DatabaseError.WithError(err), "staff_id", staff.ID.Hex(), "step", "write_login_log")
	}
	...
	slog.InfoContext(ctx, "admin.auth.login.success", "staff_id", staff.ID.Hex())
	return result, nil
}

func (s *Service) Session(ctx context.Context, principal session.Principal) (*AuthSession, error) {
	slog.InfoContext(ctx, "admin.auth.session.start")
	if principal.Terminal != session.TerminalAdmin || principal.PrincipalType != session.PrincipalTypeStaff {
		...
		applog.Result(ctx, "admin.auth.session.success", "admin.auth.session.failed", err, "principal_id", principal.PrincipalID)
		return nil, err
	}
	...
	if err != nil {
		...
		applog.Result(ctx, "admin.auth.session.success", "admin.auth.session.failed", authErr, "principal_id", principal.PrincipalID)
		return nil, authErr
	}
	staff, err := s.staffRepo.FindActiveByID(ctx, staffID)
	if err != nil {
		dbErr := publicSystemError(errcode.DatabaseError.Code, "获取登录状态失败，请稍后重试", err)
		applog.Result(ctx, "admin.auth.session.success", "admin.auth.session.failed", dbErr, "staff_id", staffID.Hex())
		return nil, dbErr
	}
	if staff == nil {
		authErr := errcode.Unauthorized.WithError(fmt.Errorf("当前登录状态已失效，请重新登录"))
		applog.Result(ctx, "admin.auth.session.success", "admin.auth.session.failed", authErr, "staff_id", staffID.Hex())
		return nil, authErr
	}
	authSession, err := s.buildSession(ctx, staff, "获取登录状态失败，请稍后重试")
	applog.Result(ctx, "admin.auth.session.success", "admin.auth.session.failed", err, "staff_id", staffID.Hex())
	return authSession, err
}

func (s *Service) Logout(ctx context.Context, token string) error {
	slog.InfoContext(ctx, "admin.auth.logout.start")
	if strings.TrimSpace(token) == "" {
		err := errcode.Unauthorized.WithError(fmt.Errorf("未登录或登录已过期，请重新登录"))
		applog.Result(ctx, "admin.auth.logout.success", "admin.auth.logout.failed", err)
		return err
	}
	...
	if err := s.sessionStore.Delete(ctx, token); err != nil {
		...
		applog.Result(ctx, "admin.auth.logout.success", "admin.auth.logout.failed", cacheErr)
		return cacheErr
	}
	slog.InfoContext(ctx, "admin.auth.logout.success")
	return nil
}
