...
func (p *MiniappProjector) RefreshByListing(ctx context.Context, listing *model.HpdListing) error {
	if listing == nil {
		return fmt.Errorf("hpd listing is nil")
	}
	switch listing.SourceType {
	case model.HpdSourceTypeCentralizedRoom:
		return p.RefreshCentralizedRoom(ctx, listing.SourceID)
	case model.HpdSourceTypeDecentralizedRoom:
...
func (p *MiniappProjector) RefreshCentralizedRoom(ctx context.Context, roomID bson.ObjectID) error {
	...
	listing, err := p.hpdListingRepo.UpsertBySource(ctx, &model.HpdListing{
