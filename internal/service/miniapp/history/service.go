...
func (s *Service) Add(ctx context.Context, input AddInput) (*AddResult, error) {
	slog.InfoContext(ctx, "miniapp.history.add.start", "user_id", input.UserID.Hex(),
		"listing_id", input.ListingID.Hex(),
		"source", input.Source,
	)
	if input.UserID.IsZero() {
		err := errcode.InvalidParam.WithErrorf("user_id is required")
		slog.WarnContext(ctx, "miniapp.history.add.failed", "error", err)
		return nil, err
	}
	if input.ListingID.IsZero() {
		err := errcode.InvalidParam.WithErrorf("listing_id is required")
		slog.WarnContext(ctx, "miniapp.history.add.failed", "user_id", input.UserID.Hex(),
			"error", err,
		)
		return nil, err
	}
	...
	if err := s.history.Upsert(ctx, &useractivitymodel.History{
		...
		slog.ErrorContext(ctx, "miniapp.history.add.failed", "user_id", input.UserID.Hex(),
			"listing_id", input.ListingID.Hex(),
			"step", "upsert_history",
			"error", wrapped,
		)
	...
	slog.InfoContext(ctx, "miniapp.history.add.success", "user_id", input.UserID.Hex(),
		"listing_id", input.ListingID.Hex(),
		"viewed_at", viewedAt,
	)
	return result, nil
...
func (s *Service) List(ctx context.Context, input ListInput) (*ListResult, error) {
	slog.InfoContext(ctx, "miniapp.history.list.start", "user_id", input.UserID.Hex(),
		"page", input.Page,
		"page_size", input.PageSize,
	)
	if input.UserID.IsZero() {
		err := errcode.InvalidParam.WithErrorf("user_id is required")
		slog.WarnContext(ctx, "miniapp.history.list.failed", "error", err)
		return nil, err
	}
	...
	if err != nil {
		...
		slog.ErrorContext(ctx, "miniapp.history.list.failed", "user_id", input.UserID.Hex(),
			"step", "list_history",
			"error", wrapped,
		)
		return nil, wrapped
	...
	slog.InfoContext(ctx, "miniapp.history.list.success", "user_id", input.UserID.Hex(),
		"page", page,
		"page_size", pageSize,
...
func (s *Service) Count(ctx context.Context, userID bson.ObjectID) (int64, error) {
	if userID.IsZero() {
		err := errcode.InvalidParam.WithErrorf("user_id is required")
		slog.WarnContext(ctx, "miniapp.history.count.failed", "error", err)
		return 0, err
	}
	...
	if err != nil {
		...
		slog.WarnContext(ctx, "miniapp.history.count.failed", "user_id", userID.Hex(),
			"error", wrapped,
		)
		return 0, wrapped
	}
	slog.InfoContext(ctx, "miniapp.history.count.success", "user_id", userID.Hex(),
		"total", result.Total,
	)
