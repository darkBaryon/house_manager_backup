...
func (s *centralizedRoomService) CreateCentralizedRoom(ctx context.Context, input CreateCentralizedRoomInput) (*hmdmodel.HmdRoomCentralized, error) {
	slog.InfoContext(ctx, "publish.room.create.start", "project_id", input.ProjectID.Hex(), "building_id", input.BuildingID.Hex(), "room_no", input.RoomNo)
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.room.create.success", "publish.room.create.failed", err)
		return nil, err
	}
	if err := s.requireBuildingAccess(ctx, scope, input.BuildingID, "create centralized room"); err != nil {
		slog.WarnContext(ctx, "publish.room.create.denied", "building_id", input.BuildingID.Hex(), "error", err)
		return nil, err
	}
	input, err = s.mergeRoomTypeTemplate(ctx, input)
	if err != nil {
		applog.Result(ctx, "publish.room.create.success", "publish.room.create.failed", err, "building_id", input.BuildingID.Hex(), "room_type_id", input.RoomTypeID.Hex(), "room_no", input.RoomNo)
		return nil, err
	}
	result, err := s.hmd.CreateCentralizedRoom(ctx, input)
	entity, err := resolveHmdMutation(ctx, s.publisher, result, err)
	if err != nil {
		applog.Result(ctx, "publish.room.create.success", "publish.room.create.failed", err, "building_id", input.BuildingID.Hex(), "room_no", input.RoomNo)
		return nil, err
	}
	slog.InfoContext(ctx, "publish.room.create.success", "room_id", entity.ID.Hex(), "project_id", entity.ProjectID.Hex(), "room_no", entity.RoomNo)
	return entity, nil
}
...
func (s *centralizedRoomService) GetCentralizedRoom(ctx context.Context, id bson.ObjectID) (*hmdmodel.HmdRoomCentralized, error) {
	slog.InfoContext(ctx, "publish.room.detail.start", "room_id", id.Hex())
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.room.detail.success", "publish.room.detail.failed", err, "room_id", id.Hex())
		return nil, err
	}
	...
	if room != nil {
		if err := requireCentralizedProjectAccess(ctx, scope, room.ProjectID, "get centralized room"); err != nil {
			slog.WarnContext(ctx, "publish.room.detail.denied", "room_id", id.Hex(), "project_id", room.ProjectID.Hex(), "error", err)
			return nil, err
		}
	}
	slog.InfoContext(ctx, "publish.room.detail.success", "room_id", id.Hex(), "found", room != nil)
	return room, nil
}
...
func (s *centralizedRoomService) ListCentralizedRoomsByProject(ctx context.Context, projectID bson.ObjectID) ([]hmdmodel.HmdRoomCentralized, error) {
	slog.InfoContext(ctx, "publish.room.list_by_project.start", "project_id", projectID.Hex())
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.room.list_by_project.success", "publish.room.list_by_project.failed", err, "project_id", projectID.Hex())
		return nil, err
	}
	...
	if !allowed {
		slog.WarnContext(ctx, "publish.room.list_by_project.denied", "project_id", projectID.Hex())
		return []hmdmodel.HmdRoomCentralized{}, nil
	}
	rooms, err := s.hmd.ListCentralizedRoomsByProject(ctx, projectID)
	applog.Result(ctx, "publish.room.list_by_project.success", "publish.room.list_by_project.failed", err, "project_id", projectID.Hex(), "result_count", len(rooms))
	return rooms, err
}
...
func (s *centralizedRoomService) ListCentralizedRoomsByBuilding(ctx context.Context, buildingID bson.ObjectID) ([]hmdmodel.HmdRoomCentralized, error) {
	slog.InfoContext(ctx, "publish.room.list_by_building.start", "building_id", buildingID.Hex())
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.room.list_by_building.success", "publish.room.list_by_building.failed", err, "building_id", buildingID.Hex())
		return nil, err
	}
	...
	if !allowed {
		slog.WarnContext(ctx, "publish.room.list_by_building.denied", "building_id", buildingID.Hex())
		return []hmdmodel.HmdRoomCentralized{}, nil
	}
	rooms, err := s.hmd.ListCentralizedRoomsByBuilding(ctx, buildingID)
	applog.Result(ctx, "publish.room.list_by_building.success", "publish.room.list_by_building.failed", err, "building_id", buildingID.Hex(), "result_count", len(rooms))
	return rooms, err
}
...
func (s *centralizedRoomService) UpdateCentralizedRoom(ctx context.Context, input UpdateCentralizedRoomInput) (*hmdmodel.HmdRoomCentralized, error) {
	slog.InfoContext(ctx, "publish.room.update.start", "room_id", input.ID.Hex())
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.room.update.success", "publish.room.update.failed", err, "room_id", input.ID.Hex())
		return nil, err
	}
	...
	if room == nil {
		...
		slog.WarnContext(ctx, "publish.room.update.denied", "room_id", input.ID.Hex(), "error", err)
		return nil, err
	}
	if err := requireCentralizedProjectAccess(ctx, scope, room.ProjectID, "update centralized room"); err != nil {
		slog.WarnContext(ctx, "publish.room.update.denied", "room_id", input.ID.Hex(), "project_id", room.ProjectID.Hex(), "error", err)
		return nil, err
	}
...
func (s *centralizedRoomService) UpdateCentralizedRoomStatus(ctx context.Context, input UpdateCentralizedRoomStatusInput) (*hmdmodel.HmdRoomCentralized, error) {
	slog.InfoContext(ctx, "publish.room.update_status.start", "room_id", input.ID.Hex(), "room_status", input.RoomStatus)
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.room.update_status.success", "publish.room.update_status.failed", err, "room_id", input.ID.Hex())
		return nil, err
	}
	room, err := s.hmd.GetCentralizedRoom(ctx, input.ID)
	if err != nil {
	...
	if room == nil {
		...
		slog.WarnContext(ctx, "publish.room.update_status.denied", "room_id", input.ID.Hex(), "error", err)
		return nil, err
	}
	if err := requireCentralizedProjectAccess(ctx, scope, room.ProjectID, "update centralized room status"); err != nil {
		slog.WarnContext(ctx, "publish.room.update_status.denied", "room_id", input.ID.Hex(), "project_id", room.ProjectID.Hex(), "error", err)
		return nil, err
	}
	result, err := s.hmd.UpdateCentralizedRoomStatus(ctx, input)
	entity, err := resolveHmdMutation(ctx, s.publisher, result, err)
	applog.Result(ctx, "publish.room.update_status.success", "publish.room.update_status.failed", err, "room_id", input.ID.Hex(), "room_status", input.RoomStatus)
	return entity, err
}
