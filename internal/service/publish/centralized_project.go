...
func (s *centralizedProjectService) CreateCentralizedProject(ctx context.Context, input CreateCentralizedProjectInput) (*hmdmodel.HmdCentralized, error) {
	slog.InfoContext(ctx, "publish.project.create.start", "project_name", input.ProjectName, "city", input.City, "district", input.District)
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.project.create.success", "publish.project.create.failed", err, "project_name", input.ProjectName)
		return nil, err
	}
	if err := s.ensureNoDuplicateProjectForLandlord(ctx, scope, input); err != nil {
		applog.Result(ctx, "publish.project.create.success", "publish.project.create.failed", err, "project_name", input.ProjectName, "city", input.City)
		return nil, err
	}
	...
	if err != nil {
		...
		applog.Result(ctx, "publish.project.create.success", "publish.project.create.failed", err, "project_id", createdID.Hex(), "project_name", input.ProjectName)
		return nil, err
	}
	if entity != nil {
		slog.InfoContext(ctx, "publish.project.create.hmd_created", "project_id", entity.ID.Hex(), "project_name", entity.ProjectName)
		if err := registerRootScope(ctx, s.access, hpdmodel.HpdRootScopeTypeCentralizedProject, entity.ID, scope.principal); err != nil {
			...
			applog.Result(ctx, "publish.project.create.success", "publish.project.create.failed", err, "project_id", entity.ID.Hex())
			return nil, err
		}
		slog.InfoContext(ctx, "publish.project.create.root_scope_created", "project_id", entity.ID.Hex())
	}
	slog.InfoContext(ctx, "publish.project.create.success", "project_id", entity.ID.Hex(), "project_name", entity.ProjectName)
	return entity, nil
}
...
func (s *centralizedProjectService) GetCentralizedProject(ctx context.Context, id bson.ObjectID) (*hmdmodel.HmdCentralized, error) {
	slog.InfoContext(ctx, "publish.project.detail.start", "project_id", id.Hex())
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.project.detail.success", "publish.project.detail.failed", err, "project_id", id.Hex())
		return nil, err
	}
	...
	if project != nil {
		if err := requireCentralizedProjectAccess(ctx, scope, project.ID, "get centralized project"); err != nil {
			slog.WarnContext(ctx, "publish.project.detail.denied", "project_id", id.Hex(), "error", err)
			return nil, err
		}
	}
	slog.InfoContext(ctx, "publish.project.detail.success", "project_id", id.Hex(), "found", project != nil)
	return project, nil
}
...
func (s *centralizedProjectService) ListCentralizedProjects(ctx context.Context, input ListCentralizedProjectsInput) ([]hmdmodel.HmdCentralized, error) {
	slog.InfoContext(ctx, "publish.project.list.start", "city", input.City, "district", input.District)
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.project.list.success", "publish.project.list.failed", err, "city", input.City, "district", input.District)
		return nil, err
	}
	projectIDs, err := scope.accessibleProjectIDs(ctx)
	if err != nil {
		applog.Result(ctx, "publish.project.list.success", "publish.project.list.failed", err, "city", input.City, "district", input.District, "step", "resolve_scope")
		return nil, err
	}
	projects, err := s.hmd.ListCentralizedProjectsByIDs(ctx, projectIDs, input)
	if err != nil {
		applog.Result(ctx, "publish.project.list.success", "publish.project.list.failed", err, "city", input.City, "district", input.District, "project_scope_count", len(projectIDs))
		return nil, err
	}
	slog.InfoContext(ctx, "publish.project.list.success", "project_scope_count", len(projectIDs), "result_count", len(projects))
	return projects, nil
}
...
func (s *centralizedProjectService) UpdateCentralizedProject(ctx context.Context, input UpdateCentralizedProjectInput) (*hmdmodel.HmdCentralized, error) {
	slog.InfoContext(ctx, "publish.project.update.start", "project_id", input.ID.Hex())
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.project.update.success", "publish.project.update.failed", err, "project_id", input.ID.Hex())
		return nil, err
	}
	if err := requireCentralizedProjectAccess(ctx, scope, input.ID, "update centralized project"); err != nil {
		slog.WarnContext(ctx, "publish.project.update.denied", "project_id", input.ID.Hex(), "error", err)
		return nil, err
	}
