...
func (s *buildingService) CreateBuilding(ctx context.Context, input CreateBuildingInput) (*hmdmodel.HmdBuilding, error) {
	slog.InfoContext(ctx, "publish.building.create.start", "project_id", input.ProjectID.Hex(), "building_name", input.BuildingName)
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.building.create.success", "publish.building.create.failed", err, "project_id", input.ProjectID.Hex())
		return nil, err
	}
	if err := s.requireProjectAccess(ctx, scope, input.ProjectID, "create building"); err != nil {
		slog.WarnContext(ctx, "publish.building.create.denied", "project_id", input.ProjectID.Hex(), "error", err)
		return nil, err
	}
...
func (s *buildingService) GetBuilding(ctx context.Context, id bson.ObjectID) (*hmdmodel.HmdBuilding, error) {
	slog.InfoContext(ctx, "publish.building.detail.start", "building_id", id.Hex())
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.building.detail.success", "publish.building.detail.failed", err, "building_id", id.Hex())
		return nil, err
	}
	...
	if building != nil {
		if err := s.requireProjectAccess(ctx, scope, building.ProjectID, "get building"); err != nil {
			slog.WarnContext(ctx, "publish.building.detail.denied", "building_id", id.Hex(), "project_id", building.ProjectID.Hex(), "error", err)
			return nil, err
		}
	}
	slog.InfoContext(ctx, "publish.building.detail.success", "building_id", id.Hex(), "found", building != nil)
	return building, nil
}

func (s *buildingService) ListBuildingsByProject(ctx context.Context, projectID bson.ObjectID) ([]hmdmodel.HmdBuilding, error) {
	slog.InfoContext(ctx, "publish.building.list_by_project.start", "project_id", projectID.Hex())
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.building.list_by_project.success", "publish.building.list_by_project.failed", err, "project_id", projectID.Hex())
		return nil, err
	}
	...
	if !allowed {
		slog.WarnContext(ctx, "publish.building.list_by_project.denied", "project_id", projectID.Hex())
		return []hmdmodel.HmdBuilding{}, nil
	}
	buildings, err := s.hmd.ListBuildingsByProject(ctx, projectID)
	applog.Result(ctx, "publish.building.list_by_project.success", "publish.building.list_by_project.failed", err, "project_id", projectID.Hex(), "result_count", len(buildings))
	return buildings, err
}

func (s *buildingService) UpdateBuilding(ctx context.Context, input UpdateBuildingInput) (*hmdmodel.HmdBuilding, error) {
	slog.InfoContext(ctx, "publish.building.update.start", "building_id", input.ID.Hex())
	scope, err := newPublishScope(ctx, s.access)
	if err != nil {
		applog.Result(ctx, "publish.building.update.success", "publish.building.update.failed", err, "building_id", input.ID.Hex())
		return nil, err
	}
	...
	if building == nil {
		...
		slog.WarnContext(ctx, "publish.building.update.denied", "building_id", input.ID.Hex(), "error", err)
		return nil, err
	}
	if err := s.requireProjectAccess(ctx, scope, building.ProjectID, "update building"); err != nil {
		slog.WarnContext(ctx, "publish.building.update.denied", "building_id", input.ID.Hex(), "project_id", building.ProjectID.Hex(), "error", err)
		return nil, err
	}
