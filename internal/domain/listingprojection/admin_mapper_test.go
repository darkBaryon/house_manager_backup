...
func TestMapCentralizedAdminListingBuildsOperationSnapshot(t *testing.T) {
	...
	got := mapCentralizedAdminListing(listing, room, project, building, roomType, owner)
	if got.ListingID != listing.ID || got.ProjectName != project.ProjectName || got.BuildingName != building.BuildingName {
		t.Fatalf("unexpected admin listing identity/name mapping: %#v", got)
	}
	...
	if got.AuditStatus != hpdmodel.HpdAuditStatusUnspecified {
		t.Fatalf("expected unspecified audit status before audit task sync, got %#v", got.AuditStatus)
	}
}

func TestMapDecentralizedAdminListingBuildsCommunitySnapshot(t *testing.T) {
	listing := &hpdmodel.HpdListing{
		CommonFields:  commonmodel.CommonFields{ID: bson.NewObjectID()},
	...
	got := mapDecentralizedAdminListing(listing, room, community, owner)
	...
	if got.CommunityName != community.CommunityName || got.RoomNo != room.RoomNo {
		t.Fatalf("unexpected admin field mapping: %#v", got)
	}
	if got.IsOnline != hpdmodel.HpdOnlineStatusYes {
		t.Fatalf("expected published available room online, got %#v", got.IsOnline)
	}
}
